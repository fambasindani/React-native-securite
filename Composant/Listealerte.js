...
const Listealerte = ({ mydata, SetLoading, Loading, fetchUserData, handleConfirm }) => {
  ...
  const renderItem = ({ item, index }) => {
    ...
    const Actionmaps = async(item) => {
      ...
      const url = ApiUrl({ endpoint: 'getlocalisation' });
     // 
      if(monid==item.recev_id){
        ...
        try {
         ...
         const response = await axios.get(`${url}/${item.envoi_id}`);
       
         //const newData = response.data;
        // Alert.alert(response.data[0].nom)
       ...
       try {
        // Alert.alert("recuperer id_envoi",item.envoi_id.toString())
        //const response = await axios.get(urlconfirmation/${item.id});
        const response = await axios.get(`${url}/${item.recev_id}`);
      ...
      <View style={styles.itemContainer}>
        ...
        <View style={styles.descriptionContainer}>
          <View style={styles.nameDateContainer}>
            ...
            <Text style={styles.date}>{timeAgo(item.datenotification) ?timeAgo(item.datenotification)  : 'N/A'}</Text>
          </View>
