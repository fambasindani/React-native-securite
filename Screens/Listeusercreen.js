...
const Listeamiscreen = ({ navigation }) => {
  ...
  // var userids = await AsyncStorage.getItem('key');
   ...
   const nextpage=currentpage+1;
    ...
    try {
      ...
      const urls = ApiUrl({ endpoint: `getadminpage` });
 ...
 /// Alert.alert(item.nom)
  ...
  const deleteUser = async (item) => {
    try {
      ...
      const url = ApiUrl({ endpoint: `delete_user/${item.id}` }); // Remplacez par l'endpoint correct
    ...
    <View style={styles.complet}>
     ...
     <View style={styles.container}>
      <View style={styles.headerContainer}>
        ...
        <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('Adduserscreen', {refresh:fetchUserData})}>
          ...
          <Messagebox  message={message} handleConfirm={handleConfirms} handleCancel={handleCancel}  modalVisible={modalVisible}  handleCloseModal={handleCloseModal}  showSuccessModal={showSuccessModal} setShowSuccessModal={setShowSuccessModal}  />
              ...
              <Listeusers handupdate={handupdate} handDelete={handDelete} mydata={filteredData}  fetchUserData={fetchUserData}   SetLoading={SetLoading} Loading={Loading}  handleConfirm={handleConfirm}   />
