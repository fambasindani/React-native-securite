...
export default function Updatealertscreen({ navigation, route }) {
  ...
  const adduser = async () => {
    ...
    //const url = ApiUrl({ endpoint: 'update_contenu' });
    const url = ApiUrl({ endpoint: `update_contenu/${userid}` });
    setloading(true)
    setTimeout(async () => {
    try {
